function init() {
  ...
  //color 생성
  var color = d3.scale.linear().domain([0, 20]).range([minColor, maxColor]);
  // svg 생성
    ...
    svg
      ...
      .on("mouseover", function (d, i) {
        ...
        var s =
          ...
          ": " +
          d.properties.rate.toString().slice(0, -3) +
          "(백만원)";
        div
          .html(s)
...
function draw_graph(gu_name) {
  ...
  var y1Axis = d3.svg.axis().scale(y1).orient("left").ticks(10);

  var y2Axis = d3.svg.axis().scale(y2).orient("right").ticks(10);
